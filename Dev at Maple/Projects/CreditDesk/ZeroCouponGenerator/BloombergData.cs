#if !NO_BB // Used to exclude Bloomberg stuff when debugging the non Bloomberg part of an app on a non Bloomberg machine
...
namespace Shared
{
    ...
    {
        public enum eRequestType { Reference, Historical, IntraDayBar, IntraDayTick, NotSet }
    ...
    public class BloombergData
    {
        ...
        private void getBloombergData(List<BloombergDataInstrument> requests)
        {
                ...
                foreach (BloombergDataInstrument bbdi in requests)
                {
                    ...
                    switch (bbdi.RequestType)
                    {
                        case BloombergDataInstrument.eRequestType.Reference:
                            ...
                            break;
                        case BloombergDataInstrument.eRequestType.Historical:
            ...
            catch (Exception ex)
            {
                ...
                throw new Exception("An error occurred whilst sending requests to Bloomberg - " + ex.ToString(), ex);
            }
        }

        private void SetReferenceProperties(Request request, BloombergDataInstrument bbdi)
        {
        ...
        private void SetIntraDayBarProperties(Request request, BloombergDataInstrument bbdi)
        {

            DateTime dt = bbdi.DateFrom;
            Datetime t = new Datetime(dt.Year, dt.Month, dt.Day, dt.Hour, dt.Minute, dt.Second, 0);
            request.Set("startDateTime", t);
            dt = bbdi.DateTo;
            t = new Datetime(dt.Year, dt.Month, dt.Day, dt.Hour, dt.Minute, dt.Second, 0);
        ...
        private void processEvent(BB.Event eventObj, BB.Session session)
        {
            ...
            {
                NLog.LogManager.GetCurrentClassLogger().Error(e.ToString());
                throw;
            }
        }
        ...
        private void processRequestDataEvent(Event eventObj, Session session)
        {
                ...
                foreach (Message msg in eventObj.GetMessages())
                {
                    ...
                    switch (msg.MessageType.ToString())
                    {
                        ...
                        case "IntradayBarResponse":
                            ...
                            if (!hasSecurityError(secDataArray, bbdi))
                            {
                                foreach (Element barData in secDataArray.Elements)
                                {
            ...
            {
                UpdateStatus("Error occurred processing reply : " + ex.ToString());
                throw;
            }
            finally
            {
        ...
        private bool hasSecurityError(Element secDataArray, BloombergDataInstrument bbdi)
        {
            bool ret = false;
        ...
        /// <returns></returns>
        private bool hasFieldError(Element secData, BloombergDataInstrument bbdi)
        {
                            ...
                            foreach (BloombergDataInstrumentField fld in bbdi.BBFields.Values)
                            {
                                ...
                                try
                                {
                                    if (fields.HasElement(key))
                                    {
                                        ...
                                        Element item = fields.GetElement(key);
        ...
        private void CheckForNoDataReturned(Element secData, BloombergDataInstrument bbdi)
        {
            if (secData.NumValues == 0)
            {  // No values returned
                foreach (BloombergDataInstrumentField fld in bbdi.BBFields.Values)
        ...
        private string processBulkData(BB.Element data)
        {
            ...
            {
                UpdateStatus("Error occurred processing array field : " + ex.ToString());
